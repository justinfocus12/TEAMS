...
class CaneZebiakGCM(DynamicalSystem):
    ...
    def derive_parameters(self, config):
        ...
        self.remove_temp = config['remove_temp'] # whether to remove temporary files involved in a single-member integration after the integration is done. Set to False for debugging purposes

        # JF2SP: replace with the name of your namelist 
        nml = f90nml.read(join(self.base_dir_absolute, 'input', 'jf_spectral_namelist')).todict()
        # Update the pre-existing namelist file with default parameters specified via the Python wrapper
        ...
        nmlpd = dict() # "namelist patch, derived" (based on fundamental parameters from config)
    ...
    # ---------------------- JF2SP: methods below are for transforming your existing file-handling system to mine -----------------
    def transform_inputs_to_icandf(self,inputs):
        t_find = inputs[0]
        run = inputs[1]
        icandf = dict({
            'init_cond': '~/el_nino/Out/base/outhst_'+run # this is the restart file from the 'ancestral' run, and should not be modified when spawning this new ru
            'frc': '/n/home09/spackman/Out/base/input_'+run+'.txt' 
            })
        return icandf

    def run_trajectory(self, icandf, obs_fun, saveinfo, root_dir):  #(inputs):
        ...
        os.chdir(join(root_dir, '/spackman/Out/base'))
        input_txt = [t_find+'\n', 'outhst_1\n', 'outhst_'+run]
    ...
    def run_trajectory_frierson(self, icandf, obs_fun, saveinfo, root_dir):
        # JF2SP: this is how it worked with the Frierson model 
